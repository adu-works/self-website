...
## v2.7.0 Product case study

`case-content.mjs` and `project-scene.mjs` present actual pages and component crops from this prototype. `project-cover.svg` is an original vector composition using the prototype's own copy; it is not a screenshot of an implemented product. Film clips remain the original procedural samples described above. No Anime.js models, images or implementation code were copied. Earlier mechanical concepts are retained only in Git history.
